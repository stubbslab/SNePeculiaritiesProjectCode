...
class ResidualMuCalculatorForOscillatingG: 
    ...
    def getDOmegaDPhi(self, phi): 
        if not(type(phi) in [list, np.ndarray]): phi= float(phi)
        return self.dOmegaDPhiOfFunction(phi)

    def getG(self, phi):
        omega = self.getOmega(phi)
        if not(type(phi) in [list, np.ndarray]): phi= float(phi)
        G = 1.0 / phi * (4.0 + 2.0 * omega) / (3.0 + 2.0 * omega)
        return G
    
    def getDG(self, phi, omega, d_omega_d_phi, d_phi):
        d_G = -d_phi / (phi ** 2.0) * (4.0 + 2.0 * omega) / (3.0 + 2.0 * omega) + 1.0 / phi * ((2.0 * d_omega_d_phi * d_phi) * (3.0 + 2.0 * omega) - (4.0 + 2.0 * omega) * (2.0 * d_omega_d_phi * d_phi)) / ((3.0 + 2.0 * omega) ** 2.0)
        return d_G
    
    
        
    def getDThetaDT(self, z, H, omega, d_omega_d_phi, theta):
        ...
        d_theta_d_t = 3.0 / (2.0 * omega + 3.0) * (self.OmM0 * (1.0 + z) ** 3.0 + 4.0 * self.OmL0) - theta * (3.0 * H + d_omega_d_phi * theta / (2.0 * omega + 3.0)) 
        #if H == 1.0: print 'At H == 1.0, [z, H, omega, phi, d_phi_d_t] =' + str([z, H, omega, phi, d_phi_d_t])
        return d_theta_d_t

    def getDHDT(self, H, phi, z, omega, d_omega_d_phi, theta): 
        #d_t_to_d_z_scaling = - (1.0 + z) * H 
        d_H_term1 = - omega * (2.0 * self.OmR0 * (1.0 + z) ** 4.0 + self.OmM0 * (1.0 + z) ** 3.0 - 2.0 * self.OmL0) / (phi * (2.0 * omega + 3.0)) 
        d_H_term2 = - 3.0 * (self.OmR0 * (1.0 + z) ** 4.0 + self.OmM0 * (1.0 + z) ** 3.0 + self.OmL0) / (phi * (2.0 * omega + 3.0)) 
        d_H_term3 = 1.0 / 2.0 * d_omega_d_phi * theta / (2.0 * omega + 3.0) * theta / phi
        d_H_term4 = - H ** 2.0
        d_H_term5 = - omega / 3.0 * theta ** 2.0 / phi ** 2.0
    ...
    def tPhiToGSystem(self, state, z):
        ...
        d_omega_d_phi = self.getDOmegaDPhi(phi) 
        d_theta_d_t = self.getDThetaDT(z, H, omega, d_omega_d_phi, theta)
        d_theta = d_theta_d_t * self.d_t_to_d_z_scaling(z, H)
        G = self.getG(phi)
        d_G = self.getDG(phi, omega, d_omega_d_phi, d_phi) 
        if self.change_SN_phys:
            #d_dL = - (self.alpha / 2.0) * self.getDLRatio(z, G, dG) / self.getLRatio(z, G) * dL + (dL / (1.0 + z)) + (1.0 + z) / (H * G ** (self.alpha / 2.0))
            d_dL = - (self.alpha / 2.0) * d_G / G * dL + (dL / (1.0 + z)) + (1.0 + z) / (H * G ** (self.alpha / 2.0))
        else:
        ...
        d_H_d_t, d_H_terms = self.getDHDT(H, phi, z, omega, d_omega_d_phi, theta)
    ...
    def runCalculationWithGivenzs(self, zs):
        ...
        self.d_omega_d_phi_of_z = [self.getDOmegaDPhi(phi) for phi in self.phi_of_z ]
        self.d_G_of_z = [self.getDG(self.phi_of_z[i], self.omega_of_z[i], self.d_omega_d_phi_of_z[i], self.d_phi_of_z[i]) for i in range(len(self.zs))]
    ...
    def __init__(self, A, nu, alpha, psi, initial_zs, 
                       ...
                       canon_mus = None, OmM0 = None, OmL0 = None, OmR0 = None, Om0 = None, H0 = None, 
                       H0_units = 'mega_years', dL_units = 'mega_parsec', change_SN_phys = 1 ):
        #Monodromic DE has a specific potential.  So that is hard coded here; not something that a use gives
        omegaOfSinglePhiFunction = lambda phi, B, phi0, alpha, nu, A, psi: (2.0 * B * abs(1 + eps - phi /phi0) ** (-alpha) * (1 - A * np.sin(nu * phi / phi0 + psi)) - 3.0 ) / 2.0
        ...
        dOmegaDPhiOfSinglePhiFunction = lambda phi, B, phi0, alpha, nu, A, psi: ((2.0 * B * (-alpha) * (-1.0 / phi0) * abs(1 + eps - phi /phi0) ** (-alpha - 1.0) * (1 - A * np.sin(nu * phi / phi0 + psi))) / 2.0 +  (2.0 * B * abs(1 + eps - phi /phi0) ** (-alpha) * (- A * nu / phi0 * np.cos(nu * phi / phi0 + psi))) / 2.0 )
        dOmegaDPhiOfFunction = lambda phis, B, phi0, alpha, nu, A, psi: ([dOmegaDPhiOfSinglePhiFunction(phis[i], B, phi0, alpha, nu, A, psi) for i in range(len(phis))] if type(phis) in [list,np.ndarray]
                                                                                      else dOmegaDPhiOfSinglePhiFunction (phis, B, phi0, alpha, nu, A, psi) ) 
        ...
        self.dOmegaDPhiOfFunction = lambda phis: dOmegaDPhiOfFunction(phis, self.B, self.phi0, self.alpha, self.nu, self.A, self.psi) 
        ...
        theta0 = 6.0 / omega0
        self.theta0 = theta0 
        d_omega_d_phi0 = self.dOmegaDPhiOfFunction(phi0)
        d_theta_d_t0 = self.getDThetaDT(initial_zs[0], H_init, omega0, d_omega_d_phi0, theta0) 
        ...
        d_H_d_t_init, d_H_init_terms = self.getDHDT(H_init, phi0, initial_zs[0], omega0, d_omega_d_phi0, theta0)
        ...
        print '[A, nu, alpha, psi, eps, omega0, phi0, theta0, d_phi0, d_theta_d_t0, d_theta0, H_init, d_H_d_t_init, d_H_init, z_init, self.omegaOfFunction(phi0),  self.dOmegaDPhiOfFunction(phi0)] = ' + str([A, nu, alpha, psi, eps, omega0, phi0, theta0, d_phi0, d_theta_d_t0, d_theta0, H_init, d_H_d_t_init, d_H_init, z_init, self.omegaOfFunction(phi0),  self.dOmegaDPhiOfFunction(phi0)])
