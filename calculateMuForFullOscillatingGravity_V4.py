...
class ResidualMuCalculatorForOscillatingG: 
    ...
    def __init__(self, A, nu, alpha, psi, C, initial_zs, 
        ...
        phi0 = phi0_min['x']
        if phi0 <= 4.0 / 5.0:
            phi0 = 4.0 / 5.0 #If phi0 is less than this value, theta0 must be complex
            G_of_eps = lambda new_eps: 1.0 / phi0  * (4.0 * kappa0_of_eps(new_eps) + 2.0) / (3.0 * kappa0_of_eps(new_eps) + 2.0)
            new_eps_min = minimize_scalar(lambda log_eps: abs(G_of_eps (10.0 ** log_eps)), method = 'bounded', bounds = eps_log_search_lims )
