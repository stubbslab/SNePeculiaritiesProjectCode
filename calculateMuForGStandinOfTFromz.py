...
class ResidualMuCalculatorForArbitraryGofT: 
    ...
    def getgOfomega(self, zeta):
        g = 1.0 + zeta /(zeta + 1.0) - 1.0 / 6.0 * (zeta) / (zeta + 1.0) ** 2.0
        return g 

    def getH(self, z, zeta):
        #print 'a = ' + str(a)
        #print 'OmL = ' + str(OmL)
        g = self.getgOfomega(zeta)
        #print 'g = ' + str(g) 
        OmM_hat = self.OmM0 / g * ((3 * zeta + 2)/(4 * zeta + 2))
        OmL_hat = self.OmL0 /g 
        return np.sqrt((1.0 + z) ** 3.0 * OmM_hat +  OmL_hat)
    
    def getGravitationalZetaOf(self, t, tau, z):
        return self.gravitationalZetaOf(t, tau, z)        
    ...
    def variableGLumDistSystem(self, state, z):
        ...
        zeta = self.getGravitationalZetaOf(t, tau, z)
        H = self.getH(z, zeta)
    ...
    def runCalculationWithGivenzs(self, zs):
        ...
        self.zetas = [self.getGravitationalZetaOf(self.tH0_of_z[i], self.tauH0_of_z[i], self.zs[i]) for i in range(len(self.zs))]
        self.H_of_z = self.getH(np.array(self.zs), np.array(self.zetas))
    ...
    def getMus(self):
        ...
        print 'len(np.log10(self.getUnitfulldLs()[1:])) = ' + str(len(np.log10(self.getUnitfulldLs()[1:])))
        print 'len(np.array(self.zetas)[1:]) = ' +str(len(np.array(self.zetas)[1:]))
        print 'len(np.array(self.zs)[1:]) = ' + str(len(np.array(self.zs)[1:])) 
        return 25.0 + 5.0 * np.log10(self.getUnitfulldLs()[1:]) + (15.0 * np.array(self.zetas)[1:]) / (4.0 *(np.array(self.zetas)[1:] + 1.0)) * np.log10(1.0 + np.array(self.zs)[1:]) #ignore first dL, as it is 0 (today) 
    ...
    def __init__(self, gravitationalZetaOf = lambda ts, taus, zs: 0.0 if type(ts) in [float, int] else -1.0 + np.zeros(np.shape(ts)), 
                 dL0 = 0.0, t0 = 0.0, tau0 = 0.0,
                 canon_mus = None, a0 = None, OmM0 = None, OmL0 = None, OmR0 = 0.0, Om0 = None, H0 = None, initial_zs = None, H0_units = 'mega_years', dL_units = 'mega_parsec'):
        ...
        self.gravitationalZetaOf = gravitationalZetaOf
