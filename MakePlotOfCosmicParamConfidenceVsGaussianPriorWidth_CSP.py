...
if __name__=="__main__":
    ...
    target_dir = 'stubbs/variableMuFits/mcmcResults/ToyModel/'
    ...
    surveys = ['SWIFT', 'ASASSN', 'CFA2', 'CFA1', 'KAITM', 'LOWZ', 'CFA4p2', 'KAIT', 'CFA3S', 'CSP', 'CFA3K', 'CFA4p1', 'PS1MD']
    cosmic_params = ['H0']
    ...
    cfc.makePlotOfPosteriorWidths(real_mcmc_data_files_dict, 0, ax = H0_ax, xlabel = xlabel, ylabel = r'$H_0$ posterior $\sigma$ (km s$^{-1}$ Mpc$^{-1}$) ' + '\n' + ' increase (CSP SNe only)', title = r'Expansion of H$_0$ posterior uncertainty ' + '\n' + 'with CSP as Only HF SNs Survey', data_load_dir = target_dir, plot_color = 'k', plot_marker = 'x', show_fig = 0, save_fig = 0, zero_uncertainties_at_zero_prior = 1, labelsize = labelsize, show_all_chain_posteriors = show_all_chains, n_points_to_lin_fit = len(mu_offsets),
    in_plot_lin_text = ['Asymptotic $\sigma_{H_0}$ Degredation: ', r' $\frac{\mathrm{km}} {\mathrm{s} \ \mathrm{Mpc}} $'],
    in_plot_loc = [0.20, 0.1], in_plot_text_size = labelsize + 4, n_burn_in = n_burn_in, dir_base = dir_base, x_scaling = x_scaling)
    #in_plot_lin_text = ['', ''],
    H0_ax.axvline(best_fit_mu_offsets[0] * x_scaling, c = 'k', linestyle = '--')
