...
if __name__=="__main__":
    ...
    mu_offsets = [0.002, 0.005, 0.008, 0.01, 0.015, 0.02, 0.025, 0.03, 0.035, 0.04, 0.06, 0.08, 0.1, 0.12, 0.14, 0.16, 0.18, 0.2, 0.25, 0.3, 0.35, 0.4, 0.45, 0.5, 0.55, 0.6]
    ...
    #mu_offsets = [0.002, 0.005, 0.008, 0.01, 0.015, 0.02]
    ...
    OmM_lims, w0_lims = [[-0.5, 0.5], [-1.0, 1.0]]
    ...
    real_mcmc_data_files_dict = {muSig:['GaussPriorWidth' + str(int(muSig * 1000)) + 'mMags_zLim_Full_MCMC_pantheon_plus_REAL' + 'H0_OmM_w0' + '_mu' + '_mu'.join(surveys) + '_NS10000_NC50_Cov' + str(real_sequence_id) + '.txt' for real_sequence_id in real_sequence_ids] for muSig in mu_offsets}
    ...
    area_contour_num = 0
    OmM_w0_mesh_axarr = cfc.makePlotOfPosteriorContours(full_mcmc_data_files_list, [2, 3], OmM_lims, w0_lims, xlabel = '$\Delta \Omega_M$', ylabel = r'$\Delta w$', plot_text = [r'$\Delta \mu_S$ priors of ' +  str(int(mu_offset * 1000)) + ' mmag' + (' ' if mu_offset == 0.001 else 's') for mu_offset in mu_offsets], contour_area_ax = OmM_w0_sig_ax, area_contour_num = area_contour_num, contour_area_x_vals = mu_offsets, area_plot_color = 'k', area_plot_marker = 'x', n_points_to_lin_fit = n_offsets_to_lin_fit, in_plot_lin_text = [r'$\frac{d \sigma_{w \times \Omega_M} } {d \sigma_{P,S}} =$ ', r' $\frac{1}{25 \ \mathrm{ mmag}}$'], in_plot_text_size = labelsize + 4, in_plot_loc = [0.2, 0.25])
    ...
    H0_sig_ax.axvline(best_fit_mu_offsets[0], c = 'k', linestyle = '--')
    H0_mean_ax.axvline(best_fit_mu_offsets[0], c = 'k', linestyle = '--')
    OmM_mean_ax.axvline(best_fit_mu_offsets[0], c = 'k', linestyle = '--')
    #w0_sig_ax.axvline(best_fit_mu_offsets[0], c = 'k', linestyle = '--')
    w0_mean_ax.axvline(best_fit_mu_offsets[0], c = 'k', linestyle = '--')
    OmM_w0_sig_ax.axvline(best_fit_mu_offsets[0], c = 'k', linestyle = '--')
