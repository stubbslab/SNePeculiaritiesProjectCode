...
class RawSNDataStorer:
    ...
    def __init__(self, fileNum, data_type = 'real', relevent_indeces = 'all', verbose = 0, SH0ES_big_M =  -19.253):
        "Value of SH0ES M is based on: https://arxiv.org/pdf/2112.04510.pdf"
        self.verbose = verbose
        data_archive = SNDataArchive()
        sn_file = data_archive.getSNDataFile(fileNum, data_type = data_type)
        #print ('sn_file = ' + str(sn_file))
        self.bigM = SH0ES_big_M
        ...
        #Artificial function, but we have simply replaced the measured mus with some mus of our artificial simulation
            ...
            self.orig_var_indeces = {'ID':0,            'CIDint':1,      'IDSURVEY':2,      'TYPE':3,              'FIELD':4,
                                     ...
                                     'biasCor_mB':45,   'biasCor_x1':46, 'biasCor_c':47,    'biasScale_muCOV':48,  'IDSAMPLE':49       }
            self.catch_vals = {}
        elif data_type.lower() in ['realplus_old', 'real_plus_old', 'realp_old', 'real_p_old', 'pantheon_plus_old']:
            self.orig_var_indeces = {'CID':0,              'CIDint':1,         'IDSURVEY':2,            'TYPE':3,              'FIELD':4,
            ...
            self.orig_var_indeces = {'CID':0,        'IDSURVEY':1,      'zHD':2,
                                     'zCMB':3,       'zHEL':4,          'RA':5,
                                     'DECL':6,        'mB':7,      'mBERR':8,
                                     'MUERR_VPEC':9, 'VPEC':10,         'VPECERR':11,
                                     'c':12,         'cERR':13,         'x1':14,
                                     'x1ERR':15,     'HOST_LOGMASS':16, 'IS_CALIBRATOR':17,
                                     'CEPH_DIST':18   }
            self.catch_vals = {}
        else:
            self.orig_var_indeces = {'ID':0,            'CIDint':1,           'IDSURVEY':2,       'TYPE':3,              'FIELD':4,
        ...
        with open(sn_file) as csvfile:
            #print ('sn_file = ' + str(sn_file))
            myreader = csv.reader(csvfile, delimiter = ' ')
            #print ('self.start_row = ' + str(self.start_row))
            for row in myreader:
                if row_num >= self.start_row:
                    row = row[self.start_column:]
                    #print ('row = ' + str(row))
