...
class ResidualMuCalculatorForOscillatingG: 
    ...
    def getDOmegaDT(self, phi, d_phi, H, z):
        if not(type(phi) in [list, np.ndarray]): phi= float(phi)
        if not(type(d_phi) in [list, np.ndarray]): d_phi= float(d_phi)
        return self.dOmegaDTOfFunction(phi, d_phi, H, z)

    def getDPhiDT(self, z, H, omega, phi):
        d_phi = 3.0 / omega * phi + np.sqrt((3.0 * phi / omega) ** 2.0 - 6.0 / omega * (phi * (self.OmM0 * (1.0 + z) ** 3.0 + self.OmR0 * (1.0 + z) ** 4.0 + self.OmL0) - H ** 2.0 * phi ** 2.0 - H * phi ** 2.0))
        return d_phi

    def getDH(self, H, phi, z, omega, d_omega_d_t, d_phi_d_t): 
        #d_t_to_d_z_scaling = - (1.0 + z) * H 
        d_H_term1 = -3.0 / phi * (self.OmR0 * (1.0 + z) ** 3.0 + self.OmL0)
        d_H_term2 = d_omega_d_t / (2.0 * omega + 3.0) * d_phi_d_t / phi
        d_H_term3 = 1.0 * H * d_phi_d_t / phi
        d_H_term4 = - 3.0 / (2.0 * omega + 3.0) * (-2.0 * self.OmR0 * (1.0 + z) ** 4.0 + self.OmM0 * (1.0 + z) ** 3.0 + 4.0 * self.OmL0) / phi
        #d_H_term5 = -2.0 * H * d_phi * d_t_to_d_z_scaling * 1.0 / phi
        d_H_term5 = omega / 2.0 * (d_phi_d_t / phi) ** 2.0
        d_H_term6 = - 3.0 * H ** 2.0
        ...
        d_H = 0.5 * self.d_t_to_d_z_scaling(z, H) * (d_H_term1 + d_H_term2 + d_H_term3 + d_H_term4 + d_H_term5 + d_H_term5 + d_H_term6)
        if H == 1.0:
            print 'At H == 1.0, d_H = ' + str(d_H) 
    ...
    def tPhiToGSystem(self, state, z):
        ...
        d_omega_d_t = self.getDOmegaDT(phi, d_phi_d_t, H, z)
        if H == 1.0:
            print '[phi, d_phi_d_t, H, z, self.getDOmegaDT(phi, d_phi_d_t, H, z)] = ' + str([phi, d_phi_d_t, H, z, self.getDOmegaDT(phi, d_phi_d_t, H, z)])
    ...
    def __init__(self, A, nu, alpha, psi, initial_zs, 
        ...
        dOmegaDTOfSinglePhiFunction = lambda phi, d_phi, B, phi0, alpha, nu, A, psi, H, z: ((2.0 * B * (-alpha) * (-1.0 / phi0) * abs(1 + eps - phi /phi0) ** (-alpha - 1.0) * (1 - A * np.sin(nu * phi / phi0 + psi))) / 3.0 +  (2.0 * B * abs(1 + eps - phi /phi0) ** (-alpha) * (- A * nu / phi0 * np.cos(nu * phi / phi0 + psi))) / 3.0 ) * d_phi * self.d_t_to_d_z_scaling(z, H)
        dOmegaDTOfFunction = lambda phis, d_phis, B, phi0, alpha, nu, A, psi, H, z: ([dOmegaDTOfSinglePhiFunction(phis[i], d_phis[i], B, phi0, alpha, nu, A, psi, H, z) for i in range(len(phis))] if (type(phis) in [list,np.ndarray] and type(d_phis) in [list,np.ndarray])
                                                                             else [dOmegaDTOfSinglePhiFunction(phis[i], d_phis, B, phi0, alpha, nu, A, psi, H, z) for i in range(len(phis))] if (type(phis) in [list,np.ndarray])
                                                                             else [dOmegaDTOfSinglePhiFunction(phis, d_phis[i], B, phi0, alpha, nu, A, psi, H, z) for i in range(len(d_phis))] if (type(d_phis) in [list,np.ndarray])
                                                                             else dOmegaDTOfSinglePhiFunction (phis, d_phis, B, phi0, alpha, nu, A, psi, H, z) ) 
        ...
        self.dOmegaDTOfFunction = lambda phis, d_phis, Hs, z: dOmegaDTOfFunction(phis, d_phis, self.B, self.phi0, self.alpha, self.nu, self.A, self.psi, Hs, z) 
        ...
        d_omega_d_t0 = self.dOmegaDTOfFunction(phi0, d_phi_d_t0, H_init, z_init)
        print '[phi0, d_phi_d_t0, H_init, z_init, self.getDOmegaDT(phi0, d_phi_d_t0, H_init, z_init), self.dOmegaDTOfFunction(phi0, d_phi_d_t0, H_init, z_init)] = ' + str([phi0, d_phi_d_t0, H_init, z_init, self.getDOmegaDT(phi0, d_phi_d_t0, H_init, z_init), self.dOmegaDTOfFunction(phi0, d_phi_d_t0, H_init, z_init)]) 
        ...
        print '[A, nu, alpha, psi, eps, phi0, d_phi_d_t0, d_phi0, H_init, d_H_init, z_init, self.omegaOfFunction(phi0),  self.dOmegaDTOfFunction(phi0, d_phi0, H_init, z_init)] = ' + str([A, nu, alpha, psi, eps, phi0, d_phi_d_t0, d_phi0, H_init, d_H_init, z_init, self.omegaOfFunction(phi0),  self.dOmegaDTOfFunction(phi0, d_phi0, H_init, z_init)])
