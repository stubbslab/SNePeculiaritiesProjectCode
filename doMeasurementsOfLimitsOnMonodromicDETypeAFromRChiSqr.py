...
import time
from calculateMuForMonodromicDEType1 import getMonodromicDECosmologyWithProperOmL 
...
if __name__ == '__main__':
    ...
    initial_a_s = np.linspace(0.0001, 1.0, 10000) 
    ...
    alpha = 0.2 
    alpha_str = r'$\mathrm{2/10}$'
    phi0_power_bounds = [-10.0, 0.0]
    ...
    function_to_limit = lambda sorted_lookBacks, A, nu: ( np.array(getMonodromicDECosmologyWithProperOmL(A, nu, alpha, initial_a_s, phi0_power_bounds).getMus(target_zs = sorted_zs))
                                                                - np.array(canon_mus) )
    ...
    nus_to_calc = np.linspace(0.0 + nu_min, 31 * nu_min, 11) # 101
    #omegas_to_calc_in_H0 = fs_to_calc_in_H0 * 2.0 * math.pi
    print 'nus_to_calc = ' + str(nus_to_calc)
    
    freq_half_width_to_find_peak = 100
    As_to_calc = np.linspace(0.01, 0.21, 5) #np.linspace(-0.45, 0.45, 91) # 91
    A_mesh, nu_mesh = np.meshgrid(nus_to_calc, As_to_calc)
    ...
    for i in range(len(As_to_calc)):
        ...
        for j in range(len(nus_to_calc)):
            element_start = time.time()
            nu = nus_to_calc[j]
            print 'Working on j ' + str(j) + ' of ' + str(len(nus_to_calc)) + ' at which nu = ' + str(nu)
            print 'Measuring distance modulus residuals... ' 
            raw_theoretical_mu_resids = function_to_limit(sorted_lookBacks, A, nu)
            ...
            plt.title('Monodromic DE TypaA A = ' + str(A) + ' nu = ' + str(nu) )
            plt.savefig('/Users/sasha/Desktop/muResidsMonodromicType1/' + 'muResidsMonodromicType1_A' + str(abs(A)) + '_nu_' + str(nu) + '.png')
            plt.close('all') 
            #plt.show() 
            ...
            if j % 1 == 0:
                print 'j ' + str(j) + ' of ' + str(len(nus_to_calc)) + ' took ' + str(element_end - element_start) + 's'
    ...
    file_name = 'ForMonodromicType1_alpha_' + str(int(np.around(alpha * 10.0, 0))) + '_10' + 'withFourier' + '_correctedDOF_Test'
    ...
    print 'threshold_r_chi_sqr_vals_noZeroBySurvey  = ' +str(threshold_r_chi_sqr_vals_noZeroBySurvey )
    chi_sqr_noZBS_contours = plt.contourf(A_mesh, nu_mesh, calc_chi_sqrs_noZBS, levels = threshold_r_chi_sqr_vals_noZeroBySurvey , colors = ['yellow','g','b','m','r','orange'], alpha = 0.5)
    plt.xticks(np.linspace(0.0, 0.003, 13) * 2.0 * math.pi / (km_s_Mpc_in_Myr * 100.0), np.linspace(0.0, 0.003, 13) * 1000.0)
    plt.xlim(min(nus_to_calc), max(nus_to_calc)) 
    label_size = 20.0
    plt.xlabel(r'$\nu_{mono}$' + r'$\sqrt{c^2 3 / 8 \pi G}$', fontsize = label_size, labelpad = label_size / 2.0)
    plt.ylabel('$A$' + r'$_{mono}$' + '(unitless)', fontsize = label_size)
    ...
    plt.text((np.max(nus_to_calc) + np.mean(nus_to_calc))/2.5, np.mean(As_to_calc), r'$\alpha$' + ' = ' + alpha_str, fontsize = label_size )
    plt.title('',  fontsize = label_size)
    ...
    contours = plt.contourf(A_mesh, nu_mesh, -1.0 * (1.0 - peak_CPB_vals_noZBS ** N_peaks) / ref_fourier_CPB_prob,
                            levels = [-1.0 * prob for prob in target_relative_fourier_probs], colors = ['yellow', 'g','b','m','r','orange'], alpha = 0.5)  
    plt.xticks(np.linspace(0.0, max(nus_to_calc), 11) * 2.0 * math.pi)
    plt.xlim(min(nus_to_calc), max(nus_to_calc))  
    label_size = 20.0
