...
class ResidualMuCalculatorForArbitraryGofT: 
    ...
    def getDGOf(self, t, tau, z, dt, dtau):
        return self.dGof(t, tau, z, dt, dtau)
    ...
    def getH(self, z, G):
        ...
        OmL_hat = self.OmL0
        OmR_hat = self.OmR0 * G
        return np.sqrt((1.0 + z) ** 3.0 * OmM_hat + (1.0 + z) ** 4.0 * OmR_hat + OmL_hat)
    
    def getDLRatio(self, z, G, dG):
        return -3.0 / 2.0 * (1.0 / G) ** (-3.0/2.0) * dG / G

    #This is how luminosity scales according to Gaztanaga et al.: L / L_0 = (G / G_0) ^ (-3/2)
    def getLRatio(self, z, G):
        return (1.0 / G) ** (-3.0/2.0)
    ...
    def __init__(self, Gof = lambda ts, taus, zs: 1.0 if type(taus) in [float, int] else 1.0 + np.zeros(np.shape(taus)),
                 dGof_dt = lambda ts, taus, zs: 0.0 if type(taus) in [float, int] else 0.0 + np.zeros(np.shape(taus)),
                 dGof_dtau = lambda ts, taus, zs: 0.0 if type(taus) in [float, int] else 0.0 + np.zeros(np.shape(taus)),
                 dGof_dz = lambda ts, taus, zs: 0.0 if type(taus) in [float, int] else 0.0 + np.zeros(np.shape(taus)),
                 dL0 = 0.0, t0 = 0.0, tau0 = 0.0,a0 = None, OmM0 = None, OmL0 = None, OmR0 = 0.0, Om0 = None, H0 = None, 
                 canon_mus = None, initial_zs = None, H0_units = 'mega_years', dL_units = 'mega_parsec', change_SN_phys = 1, alpha = -3.0 / 2.0): 
        ...
        self.Gof = Gof
        #self.dGof = lambda t, tau, z, dt, dtau: dt * dGof_dt(t, tau, z) + dtau * dGof_dtau(t, tau, z) + dGof_dz(t, tau, z)
        self.dGof = lambda t, tau, z, dt, dtau: dt * dGof_dt(t, tau, z) + dtau * dGof_dtau(t, tau, z) + dGof_dz(t, tau, z)
