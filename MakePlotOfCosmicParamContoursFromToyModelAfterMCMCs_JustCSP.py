...
if __name__=="__main__":
    ...
    mu_offsets = [0.01, 0.05, 0.1 ]
    mu_offsets = [0.01 ]
    z_lims_str = 'zLim_Full'
    #z_lims = [0.0, 0.15]
    #z_lims_str = 'zLim_0p0_0p15'
    cosmicParams_plot_file_name = 'H0OmMw0_Unertainties_VsMuOffsetPrior_' + z_lims_str + '_CSP.pdf'
    ...
    base_surveys = ['SWIFT', 'CFA2', 'KAIT', 'CFA1', 'LOWZ', 'CFA4p2', 'CFA3S', 'CSP' ,'CFA4p1'. 'CFA3K', 'PS1MD']
    all_surveys = ['A','B','C','D','E','F','G','H','I','J']

    surveys_file = '/Users/sashabrownsberger/Documents/Harvard/physics/stubbs/variableMuFits/ArtificialSurveys/ArtificialSNe_CSP.csv'
    ...
    colors = [colors_dict[survey] for survey in all_surveys]
    print ('colors = ' + str(colors))
    mu_prior_type = 'gauss'
    all_cols_to_save = []
    #surveys = ['SDSS']
    extra_surveys_to_include_in_sequence = []
    ...
    for j in range(len(extra_surveys_to_include_in_sequence)):
        ...
        save_files_dict = {mu_offset:['ToySNe_GaussPriorWidth' + str(int(1000 * mu_offset)) + 'mMags_' + z_lims_str + '_' + 'MCMC_' + 'toy_surveys' + '_REAL' + '_'.join(params_to_vary) + '_NS' + str(full_steps) + '_NC' + str(full_n_chains) + '_' + str(iter) + 'CSP.txt' for iter in range(1, n_iterations + 1)] for mu_offset in mu_offsets}
