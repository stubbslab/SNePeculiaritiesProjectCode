...
if __name__ == '__main__':
    ...
    canon_mus = ResidualMuCalculatorForArbitraryHofT(HScalingFunction = lambda ts, taus, zs: 1.0 if type(ts) in [float, int, np.float64] else [1.0 for t in ts], initial_zs = sorted_zs).getMus()  
    ...
    phase_str = r'$\mathrm{}\pi$' + r'$\mathrm{/4}$'
    #phase_str = r'$\mathrm{1/10}$'
    #phase_str = r'$\mathrm{0}$'
    
    if t_or_tau.lower() in ['tau','taus']:
        HofScaling = lambda ts, taus, zs, A, freq_like, phase: 1.0 + A * np.sin(freq_like * taus + phase)
    else:
        HofScaling = lambda ts, taus, zs, A, freq_like, phase: 1.0 + A * np.sin(freq_like * ts + phase)
    ...
    function_to_limit = lambda sorted_lookBacks, A, freq_like: (np.array(ResidualMuCalculatorForArbitraryHofT(HScalingFunction = lambda ts, taus, zs:
                                                                                                              HofScaling(ts, taus, zs, A, freq_like, phase),
    ...
    As_to_calc = np.linspace(-0.16, 0.16, 91) # 201 #101
    ...
    file_name = 'ForHofTauSin_phi_pix' + str(np.around(phase_pi_coef, 3)) + '+' + str(int(phase_shift_int)) + '_' + str(phase_shift_order) +  'withFourier' + '_correctedDOF_final1'
    ...
    plt.xlim(min(omegas_to_calc_in_H0), max(omegas_to_calc_in_H0)) 
    label_size = 20.0
    plt.xlabel(r'$f$' + r'$_H / H_0$' + ' (Gyr' + r'$^{-1}$' + ')', fontsize = label_size, labelpad = label_size / 2.0)
    plt.ylabel('$A$' + r'$_H$' + '(unitless)', fontsize = label_size)
    plt.text((np.max(omegas_to_calc_in_H0) + np.mean(omegas_to_calc_in_H0))/2.5, np.mean(As_to_calc), r'$\phi_H$' + ' = ' + phase_str, fontsize = label_size )
    plt.title('',  fontsize = label_size)
